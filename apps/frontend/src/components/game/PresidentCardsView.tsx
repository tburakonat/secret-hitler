...
import { emitPresidentDiscard } from '../../lib/socket';
import { useGameStore } from '../../stores/gameStore';
import { useLobbyStore } from '../../stores/lobbyStore';

export function PresidentCardsView() {
  const { t } = useTranslation();
  const { presidentialCards, presidentId } = useGameStore();
  const myPlayerId = useLobbyStore((s) => s.myPlayerId);

  if (myPlayerId !== presidentId || !presidentialCards) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-center text-sm font-medium text-white">
        {t('game.legislative.presidentTitle')}
      </p>
      <div className="flex items-start gap-3">
        {presidentialCards.map((card, i) => (
          <button
            key={i}
            onClick={() => emitPresidentDiscard({ cardIndex: i as 0 | 1 | 2 })}
